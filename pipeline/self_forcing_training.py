...
class SelfForcingTrainingPipeline:
    ...
    def inference_with_trajectory(
        ...
        # Step 2: Cache context feature (M5-A: supports multi-frame real prime,
        # recached block-by-block at t=0 exactly like deployment)
        current_start_frame = 0
        if initial_latent is not None:
            output[:, :num_input_frames] = initial_latent
            with torch.no_grad():
                for bs in range(0, num_input_frames, self.num_frame_per_block):
                    blk = initial_latent[:, bs:bs + self.num_frame_per_block]
                    timestep = torch.zeros(
                        [batch_size, blk.shape[1]], device=noise.device, dtype=torch.int64)
                    self.generator(
                        noisy_image_or_video=blk,
                        conditional_dict=conditional_dict,
                        timestep=timestep,
                        kv_cache=self.kv_cache1,
                        crossattn_cache=self.crossattn_cache,
                        current_start=(current_start_frame + bs) * self.frame_seq_length
                    )
