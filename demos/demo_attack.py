...
imageio.mimwrite(f"/root/autodl-tmp/demo_ledger/attack_demo_{_TAG}.mp4", va, fps=16, quality=8)
idx=[0, max(0,credit_px-8), credit_px, min(56,credit_px+10), 56]
strip = np.concatenate([va[i] for i in idx], 1)
imageio.imwrite(f"/root/autodl-tmp/demo_ledger/attack_demo_{_TAG}_strip.png", strip)
